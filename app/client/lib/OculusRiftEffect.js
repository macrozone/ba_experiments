...
export default function (renderer, options) {
  ...
  const worldFactor = (options && options.worldFactor) ? options.worldFactor : 1.0;

	// Specific HMD parameters
  let HMD = (options && options.HMD) ? options.HMD : {
  ...
  const emptyColor = new THREE.Color('black');

	// Render target
  const RTParams = { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter, format: THREE.RGBAFormat };
  let renderTarget = new THREE.WebGLRenderTarget(960, 1080, RTParams);
  ...
  const mesh = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), RTMaterial);
  ...
  this.setHMD = function (v) {
    ...
    const aspect = HMD.hResolution / (2 * HMD.vResolution);

		// Fov is normally computed with:
		//   THREE.Math.radToDeg( 2*Math.atan2(HMD.vScreenSize,2*HMD.eyeToScreenDistance) );
		// But with lens distortion it is increased (see Oculus SDK Documentation)
    const r = -1.0 - (4 * (HMD.hScreenSize / 4 - HMD.lensSeparationDistance / 2) / HMD.hScreenSize);
    distScale = (HMD.distortionK[0] + HMD.distortionK[1] * Math.pow(r, 2) + HMD.distortionK[2] * Math.pow(r, 4) + HMD.distortionK[3] * Math.pow(r, 6));
    const fov = THREE.Math.radToDeg(2 * Math.atan2(HMD.vScreenSize * distScale, 2 * HMD.eyeToScreenDistance));

		// Compute camera projection matrices
    const proj = (new THREE.Matrix4()).makePerspective(fov, aspect, 0.3, 10000);
    const h = 4 * (HMD.hScreenSize / 4 - HMD.interpupillaryDistance / 2) / HMD.hScreenSize;
    ...
    const lensShift = 4 * (HMD.hScreenSize / 4 - HMD.lensSeparationDistance / 2) / HMD.hScreenSize;
  ...
  this.render = function (scene, camera) {
    const cc = renderer.getClearColor().clone();
